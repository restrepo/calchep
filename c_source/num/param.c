  ...
  *strmen=malloc(24*npos+2);
  ...
  { 
     ...
     { char c=' '; 
       ...
       sprintf((*strmen)+pos,"%c%7s= %-14.5g",c,varName_int[k],readVar(k));
       pos+=24;
     }
  }
  (*strmen)[pos]=0;    
...
int selectParam(int x, int y, char * mess, void ** pscrPtr,  
    int sqtrS_on, int vars_on, int func_on,
       int* varPos, char * varName,int*mPos)
{
    char* strmen;
    void * pscr;
    int polar[2];
    ...
    { int i, shift=0;
      ...
      { shift=1; for(i=0;i<2;i++) if(polar[i]) shift++;   
        ...
        { case 1: if(polar[0]) {*varPos=-1; return 1;}
          case 2: if(polar[1]) {*varPos=-2; return 1;}
          case 3:               *varPos=0;     return 1;
        }
...
{ 
  ...
  {  
    ...
    {  
          strcat(name," = ");
          val=readVar(vPos);
          if(correctDouble(x,y+4,name,&val,1)) { writeVar(vPos,val); returnCode=1;}  
    }
    if(returnCode)
    { 
      void * pscr;
