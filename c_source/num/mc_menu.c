...
static int  in_setting(void)
{
  ...
  for(i=0;i<2;i++) 
  if(sf_num[i])mass[i]=sf_mass[i]; else mass[i]=doublePmass(Nsub,i+1);
...
{  int i;
   ...
   for(i=1,ms=0,E=0; i<=nin_int+nout_int;i++)
   { double m=doublePmass(Nsub,i);
     if(i>nin_int) ms+=m;
     else if(nin_int==1) E=m;
     else if(i==1) E+=sqrt(inP1*inP1+m*m);
     else if(i==2) E+=sqrt(inP2*inP2+m*m);
   } 
   if(nin_int>1) E= sqrt(E*E-(inP1-inP2)*(inP1-inP2));
   if(E <= ms) return 1;
   return 0;
}
