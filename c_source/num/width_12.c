 ...
 {     
    ...
    {  char * inP2=pinf_int(nsub,2,NULL,&N2);
       ...
       fprintf(f," %12.4E 2  %8d  %8d # Br(%-3s -> %-3s %-3s) %11.3E[Gev]\n",
        widths[nsub-1]/S,PDGstd(N2),PDGstd(N3),inP,inP2,inP3,widths[nsub-1]);
    }
 }
 fclose(f);
{ char mess[100];
...
{ 
   ...
   for(i=1;i<=nvar_int;i++)
   {   if(!strcmp(varName_int[i],"Q"))Q=(double*)va_int+i; 
       else if(!strcmp(varName_int[i],"GG")) GG=(double*)va_int+i;
   }
 
   if(GG)for(i=1;i<=nvar_int+nfunc_int;i++) 
      if(!strcmp(varName_int[i],"SC")){ SC=(double*)va_int+i; break;}
