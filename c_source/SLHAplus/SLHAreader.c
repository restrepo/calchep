#include"SLHAplus.h"

extern int  FError;

typedef   struct blockRec
{  struct blockRec* next;
   double val;
   int nkey;
   int  keys[12];
} blockRec;


typedef struct blockStr
{ struct blockStr*  next;
  char name[40]; 
  double scale;
  blockRec* dataList;
} blockStr;


typedef struct qNumberStr
{
  struct qNumberStr*  next;
  int pdg;
  int eQ3;
  int cDim;
  int spinDim;
  int neutral; 
} qNumberStr;

typedef   struct decayRec
{  struct decayRec* next;
   double Br;
   int nkey;
   int  pNum[12];
} decayRec;

typedef struct decayStr
{ struct decayStr*  next;
  int pNum; 
  double pWidth;
  decayRec* dataList;
} decayStr;


static blockStr* blockList=NULL;
static decayStr* decayList=NULL;
static qNumberStr*qNumberList=NULL;

static char*Warnings=NULL;
static int nWarnings=0;
static char creator[200],version[200];

static void cleanBlockRec(blockRec*List)
{
  while(List){ blockRec*list=List; List=List->next; free(list);}  
} 

static void cleanDecayRec(decayRec*List)
{
  while(List){ decayRec*list=List; List=List->next; free(list);}  
} 


static void cleanData(void)
{ 

  while(blockList)
  { blockStr*block=blockList; blockList=blockList->next; 
    cleanBlockRec(block->dataList);
    free(block); 
  }
  
  while(decayList)
  { decayStr*decay=decayList; decayList=decayList->next; 
    cleanDecayRec(decay->dataList);
    free(decay); 
  }
  
  while(qNumberList)
  {
    qNumberStr*qnb=qNumberList; qNumberList=qNumberList->next;
    free(qnb);
  }
  
  if(Warnings) {free(Warnings); Warnings=NULL;}
  nWarnings=0;
  creator[0]=0;
  version[0]=0;
}

static int nLine=0;

static int readNumber(char *s, int * iN, double *dN)
{ char buff[200];
  if(sscanf(s,"%d%s",iN, buff)==1) return 0;
  if(sscanf(s,"%lf%s",dN,buff)==1) return 1;
  return -1;
}

/*
  mode= 1*m1+2*m1+4*m4+8*m8

  m1  0 overwrite all;  1 keep old data
  m2  0 read DECAY      1: don't read   Decay 
  m4  0 read BLOCK      1: don't read   Blocks
  m8  0 ignore mistake  1: stop in case of mistake in input file.
  
  2 - clean and read only Blocks
  1 - keep old results  and read both BLOCKs and DECAYs  
*/

static FILE * _f_;
static char * _end_;

static int readLine(FILE*f,int size, char*buff)
{ int len,i;
  for(;;)
  {
    if(!fgets(buff,size-1,f))
    {  if(blockList==NULL && decayList==NULL)
       { char * mess=" No data in SLHA file\n";
         Warnings=(char*)malloc(strlen(mess)+1);
         sprintf(Warnings,mess);
         nWarnings=1;
         return -3;
       } else return -1;
    }                                                            
    nLine++;
    len=strlen(buff);
    if(len>0 && buff[len-1]=='\n')
    {
      buff[len-1]=0;
      if(len>1 && buff[len-2]==13) buff[len-2]=' ';
    }
    else
    { char ch;
      if(fscanf(f,"%c",&ch)==1) 
      {  
         printf("Line %d is too long\n",nLine);  
         cleanData();
         return nLine;
      }else if(len>0 && buff[len-1]==13) buff[len-1]=' ';    
    }
    for(i=0;buff[i]&&buff[i]==' ';i++);
    if(buff[i]!=0 && buff[i]!='#') 
    { if(i)
      { int k;
        for(k=i;buff[k];k++) buff[k-i]=buff[k];
        buff[k-i]=0; 
      }
      if(_end_ && buff==strstr(buff,_end_) )  return -1;
      return 0;
    }    
  }
}


static int readLn_(int size, char *buff)
{
   return readLine(_f_,size,buff);
}

int findNewBlock(int(*readLn)(int, char*),char* buff)
{   const char* block="BLOCK ";
    const char* decay="DECAY ";
    int err,i;
    for(;;)
    {  char*ch;
       err=readLn(200,buff);
       if(err) return 0;
       ch=strchr(buff,'#');
       if(ch) ch[0]=0;
       for(ch=buff;*ch && *ch==' '; ch++);
       for(i=0;i<6 && toupper(ch[i])==block[i];i++);
       if(i==6) return 1;
       for(i=0;i<6 && toupper(ch[i])==decay[i];i++);
       if(i==6) return 1;                 
    }
}

int slhaBasicReader( int mode, int (*readLn)(int, char*)  )
{
  char buff[200],name[200],rest[200];
  int n,err,m1,m2,m4,m8;
  double scale;
  
  m1=mode&1;
  m2=mode&2;
  m4=mode&4;
  m8=mode&8;
  
  nLine=0;
  FError=0;
  
  if(m1==0)cleanData();
    

  err=readLn(200,buff);
  if(err){ if(err==-1) return 0; else {FError=1; return err;}}
  
  for(;;) 
  { char *c, *block="BLOCK ";
    int L;
    c=strchr(buff,'#'); if(c) c[0]=0;
    for(L=0;L<6 && buff[L]&& toupper(buff[L])==block[L] ;L++) continue;
    if(L==6)
    { int i; 
      if( sscanf(buff+6," %s ",name)!=1)
      { 
         printf("  Line %d: block name is absent \n",nLine);
         if(m8) 
         { cleanData();
           FError=1;
           return nLine;
         }
         if(!findNewBlock(readLn,buff)) return 0;
         continue;   
      }  

      if(strlen(name)>19)
      {  printf("Too long name of BLOCK  at line %d\n",nLine);
        if(m8) 
        {  cleanData();
           FError=1;
           return nLine;
        }
        if(!findNewBlock(readLn,buff)) return 0;
        continue; 
      }
      for(i=0;name[i];i++) name[i]=toupper(name[i]);
      if(strcmp(name,"SPINFO")==0 ||strcmp(name,"DCINFO")==0 )
      { 
       for(;;)
       { err=readLn(200,buff);
         if(err){ if(err==-1) return 0; else {FError=1; return err;}}
         if(sscanf(buff,"%d",&n)!=1)  break; 
         if(n==1) sscanf(buff,"%*d %[^\n]",creator);
         else if(n==2) sscanf(buff,"%*d %[^\n]",version);
         else if(n==4)
         {  cleanData();  
            Warnings=(char*)malloc(strlen(buff)+2);
            sscanf(buff,"%*d %[^\n]", Warnings);
            nWarnings=1; 
            FError=1;                      
            return -2;
         }
         else if(n==3)
         {
           if(Warnings==NULL) 
           { Warnings=(char*)malloc(strlen(buff)+2);
             Warnings[0]=0;
           } else   
           Warnings=(char*)realloc(Warnings,strlen(Warnings)+strlen(buff)+4);
           sscanf(buff,"%*d %[^\n]", Warnings+strlen(Warnings)); 
           strcat(Warnings,"\n");
           nWarnings++;
         }     
       }
      }else if( strcmp(name,"QNUMBERS")==0) 
      { int cDim=0,eQ3=0,spinDim=0,val;
        qNumberStr * newQ =(qNumberStr*)malloc(sizeof(qNumberStr));
        newQ->next=qNumberList;
        qNumberList=newQ;
        
        if( sscanf(buff+6,"%*s %d", &val)==1) newQ->pdg=val;
        else
        {  printf("No pdg code for Qnumbers\n");
           if(m8)
           {  cleanData();
              FError=1;
              return nLine;
           }
           if(!findNewBlock(readLn,buff)) return 0;
           continue;                                                    
        }
        for(;;)
        { err=readLn(200,buff);
          if(err){if(err==-1) return 0; else {FError=1; return err;}}
          if(sscanf(buff,"%d %d",&n,&val)!=2)  break;
          switch(n)
          { case 1: newQ->spinDim=val;  break;
            case 2: newQ->eQ3=val;    break;
            case 3: newQ->cDim=val;   break;
            case 4: newQ->neutral=val;break;
            default:
            {  printf("unexpeted key for QNUMBERS\n");  
               if(m8)
               {
                 cleanData();  
                 FError=1;                      
                 return -2;
               }
               if(!findNewBlock(readLn,buff)) return 0;
               continue;        
            }
          }
        } 
      }
      else   
      { blockStr*newBlock; 
        if( sscanf(buff+6,"%*s %s", rest)==EOF) scale=-1;  
        else if(sscanf(buff+6,"%*s %*s %lf %s", &scale, rest)!=1)
        {  
           printf("Unexpected BLOCK specification at line %d\n",nLine);
           if(m8)
           {  cleanData();
              FError=1;
              return nLine;
           }
           if(!findNewBlock(readLn,buff)) return 0;
           continue;       
        }
        
        if(m4) for(;;)
        {
          err=readLn(200,buff);
          if(err){if(err==-1) return 0; else {FError=1; return err;}}
          if(!isdigit(buff[0])) break;
        }
        newBlock=(blockStr*)malloc(sizeof(blockStr));
        newBlock->next=blockList;
        strcpy(newBlock->name,name);         
        newBlock->dataList=NULL;
        newBlock->scale=scale;
        blockList=newBlock;

        for(;;)
        { int err,last,k;
          blockRec*dr;

          err=readLn(200,buff);
          if(err){if(err==-1) return 0; else {FError=1;return err;}}

          if(isalpha(buff[0])) break;
          c=strchr(buff,'#'); if(c) c[0]=0;
          
          dr=(blockRec*)malloc(sizeof(blockRec));
          dr->next=blockList->dataList;
          dr->nkey=0;

          for(last=0,k=0; ;k++)
          {  int tp,iVal;
             double dVal;
             
             if(k) c=strtok(NULL," "); else c=strtok(buff," ");
             if(!c) {if(!last) dr->nkey--;     break;}

             tp=readNumber(c,&iVal,&dVal);
             if(last || isalpha(c[0]) || tp==-1 ) 
             { printf("Unexpected token %d at line %d\n",k+1,nLine);
               free(dr); dr=NULL;
               if(m8)
               { cleanData();
                 FError=1;
                 return nLine;
               } 
               break;  
             }
             
             if(k>10) 
             { printf("Too long key sequence at line %d\n",nLine);
              free(dr);dr=NULL;
               if(m8)
               { cleanData();
                 FError=1;
                 return nLine;
               } 
               break;                
             }
              
             if(tp) 
             { last=1;
               dr->val=dVal;
             } else 
             {  
               dr->keys[dr->nkey]=iVal;
               dr->nkey++;
               dr->val=iVal;
             }      
          }
          if(dr) blockList->dataList=dr;

        }  
      }
    }else 
    { char  *decay="DECAY ";
      int pNum;
      double pWidth;
      decayStr*newDecay;
        
      for(L=0;L<6 && buff[L]&& toupper(buff[L])==decay[L] ;L++) continue;
      if(L!=6) 
      {  
        printf("Unexpected first word  in  line %d\n",nLine);
        if(m8) 
        { cleanData();
          FError=1; 
          return nLine;
        } 
        if(!findNewBlock(readLn,buff)) return 0;
        continue;  
      }
      
      c=strchr(buff,'#'); if(c) c[0]=0;
      if( sscanf(buff+6,"%d %lf %s",&pNum ,&pWidth, rest)!=2)
      {  printf("Unexpected DECAY specification at line %d\n",nLine);
         if(m8) 
         { cleanData();
           FError=1;
           return nLine;
         }
         if(!findNewBlock(readLn,buff)) return 0;
         continue;             
      }
      if(m2)
      { 
        for(;;)
        { double x;
          err=readLn(200,buff);
          if(err){ if(err==-1) return 0; else {FError=1;return err;}}
          if(sscanf(buff,"%lf",&x)!=1) break;
        }  
      } else 
      {
        newDecay=(decayStr*)malloc(sizeof(decayStr));
        newDecay->next=decayList;
        newDecay->pNum=pNum;         
        newDecay->dataList=NULL;
        newDecay->pWidth=pWidth;
        decayList=newDecay;
        for(;;)
        { int err;
          decayRec*dr;

          err=readLn(200,buff);
          if(err){ if(err==-1) return 0; else {FError=1; return err;}}
          if(isalpha(buff[0])) break;
          c=strchr(buff,'#'); if(c) c[0]=0;
          
          dr=(decayRec*)malloc(sizeof(decayRec));
          dr->next=newDecay->dataList;
          dr->nkey=0;
        
          err=sscanf(buff,"%lf %d %d %d %d %d %d %d %d %d",
          &dr->Br, &dr->nkey, dr->pNum, dr->pNum+1, dr->pNum+2,dr->pNum+3,
          dr->pNum+4, dr->pNum+5, dr->pNum+6,dr->pNum+7);
          if(err<2 || err!=dr->nkey+2)
          { 
            printf("Wrong decay record  at line %d\n",nLine);
            free(dr);
            if(m8)
            {
              cleanData();
              FError=1; 
              return nLine;
            }   
          } else newDecay->dataList=dr;
        }
      }    
    }     
  }
  return 0;
}         

int slhaRead(char *fname, int mode)
{ int res;
  _f_=fopen(fname,"r");
  _end_=NULL;  
  if(_f_==NULL) { FError=1; return -1;}
  res=slhaBasicReader(mode,readLn_);
  fclose(_f_);
  return res;
}

int slhaReadStream(FILE*f,  int mode, char * end )
{
  _f_=f;
  _end_=end;
  return slhaBasicReader(mode,readLn_);
}

static double* slhaValAddress(char * Block, int nKey, int *keys)
{
  char BLOCK[40];
  blockStr* blck=blockList;
  blockRec * dr;
  int i;
                            
  for(i=0; Block[i];i++)  BLOCK[i]=toupper(Block[i]);
  BLOCK[i]=0;
  
  while(blck && strcmp(BLOCK,blck->name)) blck=blck->next;
  if(!blck) return NULL;
  dr=blck->dataList;
  for(;dr;dr=dr->next)
  { if(dr->nkey!=nKey) continue;
    for(i=0;i<nKey;i++) if(keys[i]!=dr->keys[i]) break;
    if(i==nKey) return &dr->val; else continue;
  }
  return NULL;  
}

double slhaVal(char * Block, double Q, int nKey, ...)
{ 
  va_list ap; 
  int keys[12];
  int i;
  char BLOCK[40];
  blockStr* blck=blockList;
  blockRec * dr;

  double val[3], scale[3];
  int found[3]={0,0,0};

  if(strlen(Block)>19) {FError=1; return 0;}
      
  va_start(ap,nKey);
  for(i=0;i<nKey;i++)keys[i]=va_arg(ap, int);
  va_end(ap);

  for(i=0; Block[i];i++)  BLOCK[i]=toupper(Block[i]);
  BLOCK[i]=0;

  for(blck=blockList;  blck; blck=blck->next)if(strcmp(BLOCK,blck->name)==0)
  {  
     int pos;
     if(blck->scale < 0)    { if(found[0]) continue; else pos=0;}
     else if(blck->scale<Q) { if(found[1] && scale[1]>blck->scale) continue; else pos=1;}
     else                   { if(found[2] && scale[2]<blck->scale) continue; else pos=2;}

     dr=blck->dataList;
     for(;dr;dr=dr->next)
     { if(dr->nkey!=nKey) continue;
       for(i=0;i<nKey;i++) if(keys[i]!=dr->keys[i]) break;
       if(i==nKey) 
       {
          found[pos]=1;
          scale[pos]=blck->scale;
          val[pos]=dr->val; 
          break;
       }
     }  
  }
  
  if(found[0]==0 && found[1]==0 && found[2]==0) 
  { printf(" Block '%s', key={",BLOCK);
    for(i=0;i<nKey;i++) printf(" %d",keys[i]);
    printf("} - is absent\n"); 
    FError=1;
    return 0;
  }
  if(found[1]==0 && found[2]==0) return val[0];
  if(found[1]==0) return val[2];
  if(found[2]==0) return val[1];
  return  (val[1]*log(scale[2]/Q)+val[2]*log(Q/scale[1]))/log(scale[2]/scale[1]);  
}


int slhaValExists(char * Block, int nKey, ...)
{ 
  va_list ap; 
  int keys[12];
  double * address;
  int i;

  if(nKey>10) return 0;  
  va_start(ap,nKey);
  for(i=0;i<nKey;i++)keys[i]=va_arg(ap, int);
  va_end(ap);
  
  address=slhaValAddress(Block,nKey, keys);
  if(address) return 1; else return 0;
}

int slhaWarnings(FILE*f)
{
  if(f&&Warnings) fprintf(f,Warnings);
  return nWarnings;
}

int slhaWrite(char *fname)
{ 
  blockStr* block;
  blockRec* rec;
  decayStr* decay;
  decayRec* dr;
  qNumberStr*qList;
  
  FILE*f=fopen(fname,"w");
  if(!f) return 1;
  if(!blockList  && ! decayList) return 2;
  
  if(blockList)
  {
    fprintf(f,"BLOCK SPINFO # General Information\n");
    fprintf(f," 1 %s\n",creator);
    fprintf(f," 2 %s\n",version);
  } else 
  {
    fprintf(f,"BLOCK DCINFO # General Information\n");
    fprintf(f," 1 %s\n",creator);
    fprintf(f," 2 %s\n",version);
  } 
  
  
  
  if(nWarnings) 
  { char buff[100]; 
    char *  c;
    for(c=Warnings;;)
    {  
       sscanf(c,"%[^\n]",buff); 
       fprintf(f," 3 %s \n",buff);
       c=strchr(c,'\n');
       if(!c || c[1]==0) break;
       c++;
    } 
  } 
  
  for(block=blockList;block;block=block->next)
  {
     fprintf(f,"BLOCK %s", block->name);
     if(block->scale>0) fprintf(f," Q= %E", block->scale); 
     fprintf(f," # nc\n");
     for(rec=block->dataList;rec;rec=rec->next)
     {
       int i;
       for(i=0;i<rec->nkey; i++) fprintf(f," %d", rec->keys[i]);
       fprintf(f,"   %E  # nc\n",rec->val);  
     }            
  }

  for(decay=decayList;decay;decay=decay->next)
  {
     fprintf(f," DECAY %d %E # nc\n" , decay->pNum,decay->pWidth); 

     for(dr=decay->dataList;dr;dr=dr->next)
     {
       int i;
       fprintf(f," %E  %d ", dr->Br,dr->nkey);
       for(i=0;i<dr->nkey; i++) fprintf(f," %d", dr->pNum[i]);
       fprintf(f," # nc\n");  
     }            
  }
  
  for(qList=qNumberList;qList;qList=qList->next)
  { fprintf(f,"BLOCK QNUMBERS  %d \n" , qList->pdg);
    fprintf(f," 1  %d\n" , qList->spinDim);
    fprintf(f," 2  %d\n" , qList->eQ3);
    fprintf(f," 3  %d\n" , qList->cDim);
    fprintf(f," 4  %d\n" , qList->neutral);
  }
  
  fclose(f);
  return 0;
}

int slhaDecayExists(int pNum)
{ 
   decayStr* decay=decayList;
   for(;decay;decay=decay->next) if( decay->pNum==pNum)
   { decayRec*dr=decay->dataList;
     int n;
     for(n=0; dr; dr=dr->next, n++) continue;
     return n;
   }
   return -1;  
}

double slhaWidth(int pNum)
{  
   decayStr* decay=decayList;
   for(;decay;decay=decay->next) if( decay->pNum==pNum) return decay->pWidth;
   printf("Error: width for particle %d is unknown\n",pNum);
   FError=1;
   return 0;
}

double slhaBranch(int pNum,int N, int * nCh)
{
   decayStr* decay=decayList;
   for(;decay;decay=decay->next) if( decay->pNum==pNum)
   { decayRec*dr=decay->dataList;
     int i;
     for(N--;N&&dr;N--,dr=dr->next) continue;
     if(dr==NULL) { FError=1;return 0;}
     for(i=0;i<dr->nkey;i++) nCh[i]=dr->pNum[i];
     nCh[i]=0;
     return dr->Br;
   }
   FError=1;
   return 0;
}

int findQnumbers(int pdg,int*eQ3,int*spinDim,int*cDim,int*neutral)
{ qNumberStr*qNmb=qNumberList;
  for(; abs(qNmb->pdg) != abs(pdg); qNmb=qNmb->next);
  if(!qNmb) return 0;

  *spinDim=qNmb->spinDim; 
  *eQ3=qNmb->eQ3; 
  *cDim=qNmb->cDim; 
  *neutral=qNmb->neutral;
    
  if(pdg!=qNmb->pdg) 
  {  if(*neutral==0) return 0;
     (*eQ3)*=-1; if((*cDim)!=8) (*cDim)*=-1;
     return -1;
  }
  return 1;
}

int allQnumbers(int i, int *pdg,int*eQ3,int*spinDim,int*cDim,int*neutral)
{ qNumberStr*qNmb=qNumberList;
  int i1;
  for(i1=1;i1!=i && qNmb; i1++) qNmb=qNmb->next;
  if(!qNmb) return 0;
  *pdg=qNmb->pdg;
  *spinDim=qNmb->spinDim; 
  *eQ3=qNmb->eQ3; 
  *cDim=qNmb->cDim; 
  *neutral=qNmb->neutral;
  return 1;
}


int allBlocks(int i, int j, char * name, int *keyLen, int * key, double * val)
{
  blockStr* bList=blockList;
  blockRec* bRec;
  int i1,j1,k;
  
  for(i1=1;i1!=i && bList; i1++)  bList=bList->next;
  if(!bList) return 0;
  strcmp(name,bList->name); 
  bRec=bList->dataList;
  for(j1=1;j1!=j && bRec ;j++) bRec=bRec->next;
  if(!bRec) return 0;
  *keyLen=bRec->nkey;
  for(k=0;k<bRec->nkey;k++) key[k]=bRec->keys[k];
  *val=bRec->val;  
   
  return 1;  
}


int allDecays(int i, int j, int * pdg, int * decayLen, int * decay, double*width, double *br)
{
  decayStr* dList=decayList;
  decayRec* dRec;
  int i1,j1,k;
  for(i1=1;i1!=i && dList; i1++,dList=dList->next);
  if(!dList) return 0;
  *pdg=dList->pNum;
  *width=dList->pWidth;
  dRec=dList->dataList;
  for(j1=1;j1!=j && dRec ;j++) dRec=dRec->next;
  if(!dRec) return 0;
  *decayLen=dRec->nkey;
  for(k=0;k<dRec->nkey;k++) decay[k]=dRec->pNum[k];
  *br=dRec->Br; 
  return 1;
}
