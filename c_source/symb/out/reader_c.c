...
{  int dbl;
   ...
   switch (ch)
   {
      ...
      case '/': if(dbl) sprintf(ans,"%c%c|%s/(Real)%s",p_m,r_n,m1,m2);
		else if(m2[0] != '-')   sprintf(ans,"%c%c|%s/%s",p_m,r_n,m1,m2);
		else
...
{ int l,i;
  ...
  { const char * knownFunc = " sqrt sin cos tan asin acos atan exp log " 
    "fabs   atan2 log10 sinh cosh tanh asinh acosh atanh "
    "if printf slhaRead  slhaWarnings slhaVal slhaValExists slhaWrite " 
    "slhaDecayExists slhaBranch slhaWidth "
    "initDiagonal rDiagonal "
    "rDiagonalA MassArray MixMatrix MixMatrixU System openAppend aPrintF "; 
    char _name_[100];
    
    sprintf(_name_," %s ",name);
    
    checkNaN=1;
    if(strstr(knownFunc,_name_)==NULL &&  strstr(EXTFunc,_name_)==NULL  )
...
{  char      *p;
   ...
   if(s[0]=='"'){ p = m_alloc(strlen(s)+10); sprintf(p,"MS|%s",s); } 
   else  
   { 
     p = m_alloc(40);
     if (isdigit(s[0])) 
