...
static void  writesubroutineinit(void)
{
   ...
   fprintf(ext_h,"#include<math.h>\n");
   fprintf(ext_h,"#include<stdio.h>\n");
   readEXTFunc(ext_h);
...
{  catrec      cr;
   ...
   readvardef(archiv);
   if(viaFor) for_readpolynom(&rnum); else readpolynom(&rnum);
   
   clearvardef();

   if(!flag_12)
   {  outFileOpen("%sresults%cc%d.c",pathtouser,f_slash,diagrcount);
      labl();
      writeF("#include<math.h>\n");
      if(hPrec_) writeF("#include\"num_LD.h\"\n"); 
       else     writeF("#define Real double\n");
      writeF("#include\"num_out.h\"\n");
      writeF("#include\"num_in.h\"\n");   
   }
   if(viaFor)
   {
      writeF("extern void C%d_ext(Real * C,Real * CC);\n",diagrcount);
      writeF("void C%d_ext(Real * C,Real * CC)\n{\n",diagrcount);
   }else
   {
      writeF("extern void C%d_ext(Real * C);\n",diagrcount);
      writeF("void C%d_ext(Real * C)\n{\n",diagrcount);
   }   
   writeF("Real* V=va_ext;\n");
   pos_c= ftell(outFile); writeF("%80s\n",""); 
   nConst=write_const();
   if(viaFor)jump=for_write_const(rnum,&Ntt,&nCoeff); 
   else      {Ntt=0;nCoeff=0;}  
   deg1=cleardegnames();
   writeF("}\n"); 
   ...
   tmpNameMax=0;
   initdegnames();
   if(viaFor) writeF("extern void C%d_ext(Real*,Real*);\n",diagrcount);
   else       writeF("extern void C%d_ext(Real*);\n",diagrcount);
   writeF("extern Real F%d_ext(void);\n",diagrcount);
   writeF("Real F%d_ext(void)\n{\n",diagrcount);
   ...
   if(viaFor)
   {
     if(jump)
     {
       writeF("static const int jumps[%d]={",nCoeff);
       for(i=0;i<nCoeff;i++) {writeF("%d",jump[i]); if(i<nCoeff-1) writeF(","); else writeF("};\n");}
       free(jump);
     }
   } 
      
   if(nConst==0) nConst=1;
   writeF("static Real C[%d],CC[%d];\n",nConst,nCoeff);
   pos_c= ftell(outFile); writeF("%80s\n","");
   writeF("int i;\n");
   if(viaFor) writeF("if(CalcConst) C%d_ext(C,CC);\n",diagrcount);
   else  writeF("if(CalcConst) C%d_ext(C);\n",diagrcount);
   fortwriter("N",totnum);
   fortwriter("D",totdenum);
   if(viaFor)
   {
   if(jump)  writeF("for(i=0,R=0 ;i<%d;i++){R+=CC[i]*DPmonom[jumps[i]];}\n",nCoeff);
   else      writeF("for(i=1,R=CC[0];i<%d;i++){R+=CC[i]*DPmonom[i];  }\n",nCoeff); 
   }else fortwriter("R",rnum);
   writeF("R*=(N/D)");   
...
static int makeDPmonomMap(FILE* fd)
{
   marktp     bh;
   varptr     totnum, totdenum, rnum;
...
{  denlist    den_;
   ...
   { outFileOpen("%sresults%cd%d.c",pathtouser,f_slash,nsub);
     ...
     writeF("#include\"num_in.h\"\n");
     writeF("#include\"num_out.h\"\n");
     writeF("typedef Real (DNN)(Real * , int *);\n"); 
     
   }
   if(totDiag==0) 
   { if(flag_12) writeF("static ");else writeF("extern Real S%d_ext(Real*,int*);\n",nsub); 
     writeF("Real S%d_ext(Real * momenta,int * err)\n{",nsub); 
     writeF("  return 0;\n}\n");
     if(!flag_12) outFileClose(); 
     return;
   }
   ...
   {  int nden= nden_w+nden_0+1; 
      writeF("Real mass[%d],Q[%d];\n",nden,nden);
      writeF("Real* V=va_ext;\n");
      writeF("double width[%d];\n",nden);
      ...
      {  int m=0;
         ...
         if(den_->width) 
         {
           if(den_->stype) fprintf(outFile,"width[%d]=%s;\n",i,vararr[den_->width].alias);
           else 
           {  i+=nden_s;
             fprintf(outFile,"width[%d]=(twidth_ext)? %s : 0.;\n",i,vararr[den_->width].alias); 
           }
         }else 
         { i+=nden_w;
           writeF("width[%d]=0.;\n",i);
      ...
      writeF("*err=*err|prepDen(%d, BWrange_ext*BWrange_ext, s0max,mass,width, Q);\n",nden_w+nden_0);
   }
   else writeF("Real* V=va_ext;\n");
   release_(&mem_start);
   fd=fopen(fd_name,"rb"); 
   if(viaFor) 
   {  makeDPmonomMap(fd);
      fseek(fd,0,SEEK_SET);
      write_calcDPmonoms(outFile);
   } 
...
static void  make_pinf(void)
{
   ...
   writeF("char * pinf_ext(int nsub,int nprtcl,Real* pmass,long * num)\n{\n");
   writeF("int n;\n");
 ...
 if(sumDiag) viaFor=0;
  ...
  /* ======= End of Initialisation ====== */
   ...
   {  outFileOpen("%sresults%cservice.c",pathtouser,f_slash); 
      ...
      writeF(" Real va_ext[%d] ={0",nvars+nfunc+1);
      scanvars(2);
      writeF("};\n"); 
   }
   else  writeF("\nstatic double va_ext[%d];\n",nvars+nfunc+1);
   geninf("nin_ext",nin);
   ...
   { 
      make_den_info();
      writeF("\nCalcHEP_interface interface_ext={%d, %d, %d, varName_ext,va_ext,"
          "%d, %d, %d, &pinf_ext, polarized_ext, &calcFunc_ext, &BWrange_ext,&twidth_ext,"
          "&gtwidth_ext,&gswidth_ext, &sqme_ext,&den_info_ext,&build_cb_ext, &cb_pow_ext,"
          "&cb_nc_ext, &cb_chains_ext, &cb_coeff_ext, &destroy_cb_ext};\n", 
      hPrec_,nvars, nfunc, nin,nout,subproc_sq);
...
{  
 ...
 if(/*consLow*/ nin+nout<=5) NofDP=((nin+nout-1)*(nin+nout-2))/2-1;
 else        NofDP=((nin+nout)*(nin+nout-1))/2;
 initDPmonoms();
...
/*   archiv=fopen(ARCHIV_NAME "2","rb"); */
   ...
   writeF("#include<math.h>\n");
   writeF("#include<stdio.h>\n"); 
   if(hPrec) writeF("#include\"LDtype.h\"\n"); 
     else    writeF("#define Real double\n");
 ...
 fclose(diagrq);
 fclose(menuq);
 outFile=fopen("results/sqme.c","a");
 fprintf(outFile,"#include\"sqme.inc\"\n");
 fclose(outFile);
